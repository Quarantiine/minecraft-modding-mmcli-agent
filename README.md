...
- **Visual Pathway Patrols & Escort Hierarchy**:
  - **Dynamic Custom Hex-Color Routes**: Supports 5 built-in color channels (🟡 Gold, 🔵 Cyan, 🟢 Emerald, 🟣 Purple, 🔴 Crimson), 8 quick preset palette swatches rendering their vibrant swatch colors with active selection highlight borders, and custom 24-bit RGB hex colors (`#FFD700`, etc.) with a dedicated in-game Route Edit Modal Screen (`PatrolRouteEditModalScreen`).
...
- **Dual Mining Modes (AREA Default vs. DIRECT) & Confirmation Modal**:
  ...
  - **Mining & Building Completion Symmetry**: When mining/quarry tasks conclude, miners receive the exact same treatment as builders: teleportation to assigned perimeter waypoints along the rim of the quarry (`baseY = box.getMaxY() + 1`) or ground footprint with golden beacon fanfare beams and chimes, automatically entering defensive hold position (`sitting = true`, `guardAnchorPos` assigned, `selected = false`).
  - **Anti-Bobbing Mining Flight**: Minions share the exact same Arcane Levitation flight as builders during mining (uniform elevated hover station at `targetY + 1.25D`, zero gravity, zero-velocity freeze during swings), eliminating all bobbing and trench falls.
...
- **In-World Spatial Blueprint Capture ('DESIGN' Mode)**: Non-destructively convert any in-world structure into an autonomous minion construction blueprint:
  ...
  - **Capture Modal GUI (`BlueprintCaptureModalScreen`)**: Right-Clicking the ground or air (without Shift) with corners set opens the design modal dialog to assign custom names and optional descriptions (with live solid non-air block detection and dedicated height adjustment steppers cleanly isolated from text, and legacy category selection completely removed).
